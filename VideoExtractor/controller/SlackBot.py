...
def download_from_youtube(message, say):
    thread = say(
        thread_ts=message["ts"], text=f"動画のダウンロードを開始します。デフォルトのGoogleDriveに保存します。"
    )
    response: dict = service.download_video(
        message["text"][1:-1], save_path=config.video_save_path, save_media_type="drive"
    )
    say(
        thread_ts=thread["ts"],
...
def download_process_start(ack: Ack, view: dict, say: Say):
    ...
    save_media_type: str = view["state"]["values"]["media_type"]["selected_media_type"][
        "selected_option"
    ]["value"]
    ...
    try:
        ...
        if fmt == "mp4":
            ...
            response: dict = service.download_video(
                url, save_path=config.video_save_path, save_media_type=save_media_type
            )
        elif fmt == "mp3":
            response: dict = service.download_audio(
                url, save_path=config.audio_save_path, save_media_type=save_media_type
            )
        else:
            response = None
...
def analytics_video_start(ack: Ack, view: dict, say: Say):
    ...
    url: str = view["state"]["values"]["url"]["input"]["value"]
    scene_detector: str = view["state"]["values"]["scene_detector"][
        "selected_scene_detector"
    ]["selected_option"]["value"]
    numeric_threshold: str = view["state"]["values"]["numeric_threshold"][
        "input_numeric_threshold"
    ]["value"]
        ...
        if scene_detector == "numeric":
            response = service.cut_and_detect(
                ...
                save_path=os.path.join(
                    config.analytics_result_base_path, timestamp_str
                ),
                threshold=float(numeric_threshold)
                * 100,  # MAEは0-100の範囲で出力されるため、顔認識の場合と合わせるために100倍する
            )
        elif scene_detector == "face":
            face_image_url: str = view["state"]["values"]["face_image_url"][
                "face_image_url_text_input"
            ]["value"]
