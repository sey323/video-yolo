...
from src.miterator import Youtube
from src.util import Dumper
...
def download_video(
    ...
    yt_title = Youtube.download_video(url, save_path)
    return yt_title
...
def download_audio(
    ...
    yt_title = Youtube.download_audio(url, save_path)
    return yt_title
...
def cut_and_detect(
    ...
    Args:
        ...
        thres (float): シーン検出に用いる閾値
        label_clip (bool): オリジナル画像からラベル画像領域を画像として保存する場合(true)
    """
    # ExcelDumperの初期化
    HTML_dumper = Dumper(save_path=save_path, img_size=(640, 360))

    # パラメータの初期化
    picsize, frame_cnt = (640, 360), 0
    frame_penult, frame_ultima = None, np.zeros((*picsize[::-1], 3))

    # 1秒ごとに画像を処理する．
    for frame, time in Youtube(url, save_root_path=save_path, size=None):
        ...
        if cut_dct(frame_ultima, frame_penult) >= thres:  # 閾値よりMAEが大きい場合、カットと判定
            ...
            if label_clip:
                original_size_ratio = tuple(
                    [x / y for (x, y) in zip(frame.shape[:2], picsize[::-1])]
                )
                # 検出された領域のみ切り抜いて保存する
